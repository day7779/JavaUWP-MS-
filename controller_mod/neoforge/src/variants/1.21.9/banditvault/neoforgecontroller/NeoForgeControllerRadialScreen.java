...
public final class NeoForgeControllerRadialScreen extends Screen {
    ...
    public NeoForgeControllerRadialScreen() {
        super(Component.literal("Bandit Radial"));
    ...
    public void render(GuiGraphics context, int mouseX, int mouseY, float delta) {
        ...
        for (int slot = 0; slot < 8; slot++) {
            ...
            drawTile(context, x, y, size, active, slot, NeoForgeControllerCompat.radialKeyGlyph(settings.radialSlot(slot)));
        }

        String selected = selectedSlot < 0
            ? "Point with right stick"
            : NeoForgeControllerCompat.radialKeyLabel(settings.radialSlot(selectedSlot));
        ...
        context.drawCenteredString(this.font, Component.literal(shorten(selected, 36)), centerX, captionY, 0xFFFFFFFF);
        context.drawCenteredString(this.font, Component.literal(hint), centerX, captionY + 12, MUTED);
    }
    ...
    private void drawTile(GuiGraphics context, int x, int y, int size, boolean active, int slot, String glyph) {
        ...
        if (icons[slot] != null) {
            context.renderFakeItem(icons[slot], x + (size - 16) / 2, y + (size - 16) / 2);
        } else {
            String shortGlyph = shorten(glyph, 4);
            context.drawCenteredString(this.font, Component.literal(shortGlyph), x + size / 2, y + size / 2 - 4, active ? 0xFFFFFFFF : 0xFFE0E4E7);
        }
    }
    ...
    private static ItemStack iconFor(String keyId) {
        ...
        return NeoForgeRadialApi.icon(item);
    }
